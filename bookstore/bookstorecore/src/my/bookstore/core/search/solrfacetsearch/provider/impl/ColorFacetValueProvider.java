...
public class ColorFacetValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider
{
	...
	{
		final BookstoreStyleVariantProductModel BookstoreStyleModel = getBookstoreStyleProductModel(model);
		if (BookstoreStyleModel == null)
		{
			return Collections.emptyList();
		}

		final Set<SwatchColorEnum> colors = BookstoreStyleModel.getSwatchColors();
	...
	protected BookstoreStyleVariantProductModel getBookstoreStyleProductModel(final Object model)
	{
		Object finalModel = model;
		if (model instanceof BookstoreSizeVariantProductModel)
		{
			finalModel = ((BookstoreSizeVariantProductModel) finalModel).getBaseProduct();
		}

		if (finalModel instanceof BookstoreStyleVariantProductModel)
		{
			return (BookstoreStyleVariantProductModel) finalModel;
		}
		else
		{
			return null;
